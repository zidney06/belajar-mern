...
export default function CreatePage() {
	...
	useEffect(() => {
		getFetch("http://localhost:3000/api/product/my-product").then((res) => {
			if (!res.success) {
...
<<<<<<< HEAD
			...
			putfetch(`http://localhost:3000/api/product/${id}`, formData).then(
				(res) => {
					if (!res.success) {
...
<<<<<<< HEAD
			...
			postFetch("http://localhost:3000/api/product", formData).then((res) => {
				if (!res.success) {
					alert(res.response.data.message);
	...
	const hndlDelete = (index, id) => {
		if (confirm("Apakah yakin ingin dihapus?")) {
			delFetch(`http://localhost:3000/api/product/${id}`).then((res) => {
				if (!res.success) {
